...
export class ABNData extends LitElement {
  ...
  async _callWebService() {
    try {
      ...
      this.ABNDetails = { ABN: dataResult.Abn, entity: dataResult.EntityTypeName,CompanyName: dataResult.EntityName, GST: dataResult.Gst == null ? '' : dataResult.Gst };
      ...
      const event = new CustomEvent('ntx-value-change', {
        ...
        cancelable: false,
        composed: true,
        detail: this.ABNDetails
      });
      this.dispatchEvent(event);
    } catch (error) {
      this.ABNDetails = `Error: ${error.message}`;
    }
  }
...
const elementName = 'abn-data';
customElements.define(elementName, ABNData);