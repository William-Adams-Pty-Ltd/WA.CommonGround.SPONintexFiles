...
export class InventoryPlugin extends LitElement {

  static getMetaConfig() {
    return {
      controlName: 'Inventory Purchasing Form Plugin',
  ...
  _getInventoryBudgetInfoUrl(cea) {
    return `${this.apiBaseUrl}/GetInventoryBudegtInfo?purchaseNumber='${encodeURIComponent(cea)}'`;
  }
  ...
  async _loadBudget() {
    ...
    this._error = '';
    this._summary = null;
this.Url = `${this.apiBaseUrl}/GetInventoryBudegtInfo?purchaseNumber='${encodeURIComponent(cea)}'`;
    ...
    try {
     /* const settingsRes = await fetch(
      ...
      const budgetRes = await fetch(this.Url,
        { headers: { Accept: 'application/json' } }
      );
...
*/
this._summary = budgetJson;
      this.jsonOut = JSON.stringify(budgetJson);
    ...
    return html`
      ...
      ${this._open ? html`
        <div class="overlay" @click=${e => e.target === e.currentTarget && this._close()}>
          <div class="box">
            <div class="hdr">
              Budget summary: ${this._summary?.PurchasingNumber || ''}
            ...
            ` : this._summary ? html`
              <table>
                <tr class="yellow"><td colspan="2">${this._summary.PurchasingNumber}</td></tr>
                <tr><td>Total Budget</td><td>${this._summary.Budget}</td></tr>
                <tr><td>Purchasing Type</td><td>${this._summary.PurchasingType}</td></tr>
                <tr><td>Cost Center</td><td>${this._summary.CostCenter}</td></tr>
                <tr><td>Description</td><td>${this._summary.Description}</td></tr>
                <tr><td>Justification</td><td>${this._summary.Justification}</td></tr>
                <tr><td>Reporting Level</td><td>${this._summary.ReportingLevel}</td></tr>

                <tr><td colspan="2">Less: Capex raised</td></tr>
                <tr><td>&nbsp;&nbsp;Approved</td><td>${this._summary.ApprovedBudget}</td></tr>
                <tr><td>&nbsp;&nbsp;Pending</td><td>${this._summary.PendingApprovalBudget}</td></tr>
                <tr class="footer"><td>Remaining Balance</td><td>${this._summary.RemainingBalance}</td></tr>
              </table>
            ` : ''}
          </div>
        </div>
...
customElements.define('inventory-plugin', InventoryPlugin);