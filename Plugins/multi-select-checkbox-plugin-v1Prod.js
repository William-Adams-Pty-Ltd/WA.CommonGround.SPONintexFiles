...
export class MultiSelectSegmentPluginV1Prod extends LitElement {

  static getMetaConfig() {
    return {
      controlName: 'Dynamic MultiSelect Checkbox Prod',
  ...
  async _loadOptions() {
    try {
      if (this.apiUrl) {
        ...
        // CASE A: JSON array format â†’ ["ZZ:true","01:false"] or ["ZZ","01"]
        if (raw.startsWith('[')) {
          let arr = [];
          try {
            arr = JSON.parse(raw);
        ...
        // CASE B: Plain comma-separated format â†’ 01:true,02:true,04:false
        else {
...
customElements.define('multi-select-checkbox-prod', MultiSelectSegmentPluginV1Prod);